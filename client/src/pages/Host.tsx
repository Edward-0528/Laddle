...
const Host = () => {
  ...
  const testConnection = () => {
    ...
    socket.emit('test:ping', (response: any) => {
      ...
      if (response.success) {
        alert(`✅ Connection successful! Server responded: ${response.message}`)
      } else {
  ...
  const createGame = () => {
    ...
    console.log('📝 Questions to send:', questions.length)
    console.log('🔌 Socket connected:', socket.connected)
    console.log('� Socket ID:', socket.id)
    
    setIsLoading(true)
    
    // Add timeout for stuck requests
    const timeout = setTimeout(() => {
      console.error('⏱️ Game creation timed out')
      setIsLoading(false)
      alert('Connection timeout. The server may be sleeping or unreachable. Please try again in a moment.')
    }, 15000) // 15 second timeout (longer for sleeping servers)
    
    console.log('📤 Emitting host:create event...')
    socket.emit('host:create', { questions }, (response: { code?: string, error?: string }) => {
      ...
      if (response.code) {
        console.log('✅ Game created successfully with code:', response.code)
        navigate(`/game/${response.code}?host=true`)
      } else {
        console.error('❌ No game code received:', response)
